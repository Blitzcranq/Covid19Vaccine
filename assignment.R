...
# Identify the top N vaccines and group the rest as "Other"
top_vaccines <- data %>%
  group_by(vaccines) %>%
  summarise(total_vaccinations = sum(total_vaccinations, na.rm = TRUE)) %>%
  top_n(top_n_vaccines, wt = total_vaccinations) %>%
  mutate(vaccines = ifelse(row_number() <= top_n_vaccines, as.character(vaccines), "Other"))

# Filter the original data to include only the top 5vaccines and "Other"
data_filtered <- data %>%
  filter(vaccines %in% top_vaccines$vaccines | is.na(vaccines))

# Create a pie chart for the top 5 vaccines and "Other"
pie_chart_top_vaccines <- data_filtered %>%
  ggplot(aes(x = "", y = total_vaccinations, fill = vaccines)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y") +
  labs(title = paste("Top", top_n_vaccines, "Vaccines and 'Other'"),
       fill = "Vaccine") +
  theme_minimal() +
